...
export function EmbedChatPanel({
  ...
  async function send(userMessage: string) {
    ...
    const res = await fetch("/api/embed/chat/send", {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ token, conversationId, userMessage })
    }).catch(() => null);
    ...
    if (res.status === 429) {
      setIsTyping(false);
      setMessages((prev) => prev.map((m) => (m.id === tempId ? { ...m, status: "failed" as const } : m)));
      toast.error(json?.message ?? "Te veel berichten. Probeer het zo nog eens.");
      return;
    }
    ...
    if (!res.ok) {
      setIsTyping(false);
      setMessages((prev) => prev.map((m) => (m.id === tempId ? { ...m, status: "failed" as const } : m)));
      toast.error(json?.message ?? "Er ging iets mis. Probeer opnieuw.");
      return;
    }
